...
def get_tagged_atoms_from_mol(mol):
    ...
    for atom in mol.GetAtoms():
        if atom.HasProp('molAtomMapNumber'):
            atoms.append(atom)
            # print(atom.GetProp('molAtomMapNumber'))
...
def find_map_num(mol, mapnum):
    return [(a.GetIdx(), a) for a in mol.GetAtoms() if a.HasProp('molAtomMapNumber') and a.GetProp('molAtomMapNumber') == str(mapnum)][0]
...
def get_changed_atom_neigbours(mol, tag):
    for atom in mol.GetAtoms():
        if atom.HasProp('molAtomMapNumber'):
            if atom.GetProp('molAtomMapNumber') == tag:
                return atom.GetNeighbors()
    return None

def generate_leaving_groups(mol, tags):
    mw = Chem.RWMol(mol)
    for atom in mol.GetAtoms():
        print(atom.GetSymbol())
        if atom.HasProp('molAtomMapNumber'):
            tag = atom.GetProp('molAtomMapNumber')
            if tag in tags:
                mw.RemoveAtom(atom.GetIdx())
    leaving_group_mol = mw.GetMol()
    return leaving_group_mol

def has_bond(reactant, leaving_group, tags):
    for tag in tags:
        pass



def justice_leaving_groups(reactants, products, change_atom_tags):
    product_changed_atoms = []
    leaving_frags_list = []
    for product in products:

        for atom in product.GetAtoms():
            if atom.HasProp('molAtomMapNumber'):
                if atom.GetProp('molAtomMapNumber') in change_atom_tags:
                    print(atom.GetIdx())
                    product_changed_atoms.append(atom)
        bs = [atom.GetIdx() for atom in product_changed_atoms]
        if len(bs) != 2:
            return []

        # fragments = Chem.FragmentOnBonds(product, bs, addDummies=False)
        # print(Chem.MolToSmiles(fragments))
        # fragments_add_dummy = Chem.FragmentOnBonds(product, bs, addDummies=True)
        # print(Chem.MolToSmiles(fragments_add_dummy))
        # frag_mol_list = Chem.GetMolFrags(fragments_add_dummy, asMols=True, sanitizeFrags=False)
        p_smile = Chem.MolToSmiles(product)
        m = Chem.MolFromSmiles(p_smile)
        mol = Chem.RWMol(m)
        mol.RemoveBond(*bs)
        frag_mol_list = Chem.GetMolFrags(mol, asMols=True, sanitizeFrags=False)



        for testmol in frag_mol_list:
            print(Chem.MolToSmiles(testmol))
        # print(Chem.MolToSmiles(fragments))
        # print(Chem.MolToSmiles(fragments_add_dummy))
        if len(frag_mol_list) != 2:
            return []

        for frag_mol in frag_mol_list:
            # print(Chem.MolToSmiles(frag_mol))
            frag_mol_tags = set([atom.GetProp('molAtomMapNumber') for atom in frag_mol.GetAtoms() if atom.HasProp('molAtomMapNumber')])
            changed_tag = [el for el in change_atom_tags if el in frag_mol_tags][0]
            for reactant in reactants:
                info_dic = {}
                reactant_mol_tags = set([atom.GetProp('molAtomMapNumber') for atom in reactant.GetAtoms() if atom.HasProp('molAtomMapNumber')])
                if frag_mol_tags & reactant_mol_tags:
                    if reactant.HasSubstructMatch(frag_mol):
                        leaving_group = Chem.DeleteSubstructs(reactant, frag_mol)
                        if Chem.MolToSmiles(leaving_group) is '':
                            leaving_group_smi = 'H'
                            info_dic['smiles'] = Chem.MolToSmiles(reactant)
                            info_dic['frags_smiles'] = Chem.MolToSmiles(frag_mol)
                            info_dic['leaving_group_smiles'] = leaving_group_smi
                            info_dic['leaving_group_site_idx'] = None
                            info_dic['frag_changed_tag'] = changed_tag
                            info_dic['leaving_group_atomic_num'] = 1

                            leaving_frags_list.append(info_dic)
                            # leaving_frags_list.append(leaving_group_smi)

                        else:
                            for end_atom in leaving_group.GetAtoms():
                                print(end_atom.GetIdx(), end_atom.GetSymbol())
                                leaving_end_atom_idx = end_atom.GetIdx()
                                leaving_end_atomic_number = end_atom.GetAtomicNum()
                                for r_tag in change_atom_tags:
                                    for atom in reactant.GetAtoms():
                                        if atom.HasProp('molAtomMapNumber'):
                                            if atom.GetProp('molAtomMapNumber') == r_tag:
                                                whole_molecule_atom_idx = atom.GetIdx()
                                                if reactant.GetBondBetweenAtoms(leaving_end_atom_idx, whole_molecule_atom_idx):
                                                    info_dic['smiles'] = Chem.MolToSmiles(reactant)
                                                    info_dic['frags_smiles'] = Chem.MolToSmiles(frag_mol)
                                                    info_dic['leaving_group_smiles'] = Chem.MolToSmiles(leaving_group)
                                                    info_dic['leaving_group_site_idx'] = leaving_end_atom_idx
                                                    info_dic['frag_changed_tag'] = r_tag
                                                    info_dic['leaving_group_atomic_num'] = leaving_end_atomic_number
                                                    leaving_frags_list.append(info_dic)
                    else:
                        return []
                                            # print(reactant.GetBondBetweenAtoms(leaving_end_atom_idx, whole_molecule_atom_idx))

        return leaving_frags_list
...
def get_leaving_groups(reactants, products):
    ...
    changed_atoms_not_equivalent = [] # actual reactant atom species
    changed_atom_tags_not_equivalent = [] # atom map numbers of those atoms
    ...
    for i, prod_tag in enumerate(prod_atom_tags):
        ...
        for j, reac_tag in enumerate(reac_atom_tags):
            ...
            if reac_tag not in changed_atom_tags_not_equivalent: # don't bother comparing if we know this atom changes
                # If atom changed, add
                if atoms_are_different(prod_atoms[i], reac_atoms[j]):
                    changed_atoms_not_equivalent.append(reac_atoms[j])
                    changed_atom_tags_not_equivalent.append(reac_tag)
                    break
                ...
                if prod_atom_tags.count(reac_tag) > 1:
                    ...
                    changed_atom_tags_not_equivalent.append(reac_tag)
                    break
    leaving_groups_info = justice_leaving_groups(reactants, products, changed_atom_tags_not_equivalent)
    return leaving_groups_info
    # changed_atoms_info['not_equivalent'] = {'atoms': changed_atoms_not_equivalent, 'tags': changed_atom_tags_not_equivalent}
    #
    # changed_atoms_leaving_groups = []
    # changed_atom_tags_leaving_groups = []
    # # Reactant atoms that do not appear in product (tagged leaving groups)
    # for j, reac_tag in enumerate(reac_atom_tags):
    #     if reac_tag not in changed_atoms_info['not_equivalent']['tags']:
    #         if reac_tag not in prod_atom_tags:
    #             changed_atoms_leaving_groups.append(reac_atoms[j])
    #             changed_atom_tags_leaving_groups.append(reac_tag)
    #
    # changed_atoms_info['leaving_group'] = {'atoms': changed_atoms_leaving_groups, 'tags': changed_atom_tags_leaving_groups}
    #
    # # Atoms that change CHIRALITY (just tetrahedral for now...)
    # changed_atoms_tetro = []
    # changed_atom_tags_tetro = []
    ...
    #
    #     changed_atom_tags = changed_atoms_info['not_equivalent']['tags'] + changed_atoms_info['leaving_group']['tags']
    ...
    #                 changed_atom_tags_tetro.append(atom_tag)
    #                 changed_atoms_tetro.append(ar)
    #             else:
    #                 if VERBOSE:
    #                     print('-> adj far from reaction center, not including')
    # changed_atoms_info['tetro'] = {'atoms': changed_atoms_tetro, 'tags': changed_atom_tags_tetro}
    # [clear_isotope(reactant) for reactant in reactants]
    # [clear_isotope(product) for product in products]
    #
    # changed_atoms = changed_atoms_info['not_equivalent']['atoms'] + changed_atoms_info['leaving_group']['atoms'] + \
    #                 changed_atoms_info['tetro']['atoms']
    # changed_atom_tags = changed_atoms_info['not_equivalent']['tags'] + changed_atoms_info['leaving_group']['tags'] + \
    #                     changed_atoms_info['tetro']['tags']
    # if VERBOSE:
    #
    ...
    #         print('  {}'.format(smarts))
    #
    # return changed_atoms_info, changed_atoms, changed_atom_tags, err
    ...
    for mol in mols:
        ...
        for atom in mol.GetAtoms():
            ...
            if ':' in atom.GetSmarts():
                ...
                print(atom.GetSmarts().split(':')[1][:-1])
