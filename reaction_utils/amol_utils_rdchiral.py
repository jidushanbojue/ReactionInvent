...
import reaction_utils.coley_utils_rdchiral as cc
...
class Reaction:
	...
	def generate_reaction_template(self, radius=1):
		...
		try:
			...
			changed_atoms_info, changed_atoms, changed_atom_tags, err = cc.get_changed_atoms(reactants=reactants, products=products)
			leaving_groups_info = cc.get_leaving_groups(reactants=reactants, products=products)
			if err:
				...
				pass
			self.changed_atoms_without_leaving_groups = changed_atoms_info['not_equivalent']['atoms'] + changed_atoms_info['tetro']['atoms']
			self.changed_atom_tags_without_leaving_groups = changed_atoms_info['not_equivalent']['tags'] + changed_atoms_info['tetro']['tags']

			# Get fragments for reactants
			reactant_fragments, intra_only, dimer_only = cc.get_fragments_for_changed_atoms(reactants,
																							changed_atom_tags,
			...
			product_fragments, _, _ = cc.get_fragments_for_changed_atoms(products, changed_atom_tags,
																		 radius=radius,
																		 expansion=cc.expand_changed_atom_tags(
		...
		return self.canonical_template, self.retro_template, self.changed_atoms_without_leaving_groups, self.changed_atom_tags_without_leaving_groups, self.reactant_fragments, self.product_fragments, leaving_groups_info
	...
	def check_retro_template_outcome(self, template, products, save_outcome=True):
		...
		outcomes = rdchiralRunText(template, products)
		if save_outcome:
...
class Parsers:
	...
	def import_USPTO(self, data):
		...
		df = pd.read_csv(data, header=None, names=['rsmi', 'patent_id', 'classification_id', 'classification', 'para_num', 'year', 'data_source', 'idx'])

		USPTO_data = pd.DataFrame(columns=['rsmi', 'ID', 'classification_id', 'classification'])
		for index, row in df.iterrows():
			try:
				ID = str(row['patent_id']) + ";" + str(row['para_num']) + ";" + str(row['year']) + ';' + row['data_source'] + ';' + str(row['idx'])
				#print(row['rsmi'])
				if len(row['rsmi'].split()) == 0:
					continue
				# line = [row['rsmi'].split()[0], ID, float('nan'), row['classification']]
				line = [row['rsmi'].split()[0], ID, row['classification_id'], row['classification']]
